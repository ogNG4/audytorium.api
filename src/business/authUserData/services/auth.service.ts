...
import { CreateAccountInputDto, SigInputDto, VerifyAccountExistsInputDto } from '../types';
import { ConflictException } from '@nestjs/common';
import { UserService } from './user.service';
import * as crypto from 'crypto';
import { JwtService } from '@nestjs/jwt';
import { Response } from 'express';
import { Token } from '~/utils/auth/types/token';
@Injectable()
export class AuthService {
    ...
    async verifyUserExists(payload: VerifyAccountExistsInputDto) {
        const user = await this.userService.findOneByEmail(payload.email);
        if (user) throw new ConflictException('User already exists');
    }

    async createAccount(payload: CreateAccountInputDto) {
        const userExist = await this.userService.findOneByEmail(payload.email);
